...
// $conn = new mysqli("localhost", "root", "", "lourdes_attendance");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (empty($errors)) {
        if ($action === 'create') {
            ...
            // $stmt = $conn->prepare("INSERT INTO students (lrn, first_name, middle_name, last_name, birthdate, gender, address, parent_name, parent_email, parent_contact, status, created_at) VALUES (?, ?, ?, ?, ?, ?, 'Active', NOW())");
            // $stmt->bind_param("ssssssssss", $lrn, $first_name, $middle_name, $last_name, $birthdate, $gender, $address, $parent_name, $parent_email, $parent_contact);
            // if ($stmt->execute()) { $success_msg = "Student registered successfully!"; }
            
            $success_msg = "Student registered successfully! (Demo mode)";
        }
        
        if ($action === 'update') {
            // UPDATE QUERY PLACEHOLDER
            // $stmt = $conn->prepare("UPDATE students SET first_name=?, last_name=?, ... WHERE lrn=?");
            $success_msg = "Student updated successfully! (Demo mode)";
        }
        
        if ($action === 'delete') {
            // DELETE QUERY PLACEHOLDER
            // $stmt = $conn->prepare("DELETE FROM students WHERE lrn=?");
            $success_msg = "Student deleted successfully! (Demo mode)";
        }
    } else {
...
// DEMO DATA - Remove when database is connected
$students = [
    ['lrn'=>'123456789012','first_name'=>'Juan','middle_name'=>'Santos','last_name'=>'Dela Cruz','gender'=>'Male','birthdate'=>'2010-05-15','address'=>'Davao City','parent_name'=>'Maria Dela Cruz','parent_email'=>'maria@email.com','parent_contact'=>'09171234567','status'=>'Active'],
    ['lrn'=>'123456789013','first_name'=>'Maria','middle_name'=>'Lopez','last_name'=>'Santos','gender'=>'Female','birthdate'=>'2010-08-22','address'=>'Davao City','parent_name'=>'Jose Santos','parent_email'=>'jose@email.com','parent_contact'=>'09187654321','status'=>'Active'],
];
?>
      ...
      <div class="modal-content border-0 shadow-lg rounded-4">
        ...
        <form id="studentForm" method="POST" class="needs-validation" novalidate>
          ...
          <div class="modal-body p-4" style="max-height:70vh; overflow-y:auto;">
            ...
            <h6 class="mb-3" style="color:#0d2b5e;"><i class="bi bi-person-badge me-2"></i>Personal Information</h6>
            ...
            <h6 class="mb-3" style="color:#0d2b5e;"><i class="bi bi-people me-2"></i>Parent / Guardian Information</h6>
            <div class="row g-3">
              <div class="col-md-12">
